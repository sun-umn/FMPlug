...
# Unet copied from "A Variational Perspective on Diffusion-Based Generative
# Models and Score Matching", NeurIPS 2021, Huang et al
...
class SelfAttention(nn.Module):
    ...
    https://github.com/voletiv/self-attention-GAN-pytorch/blob/master/sagan_models.py#L29
...
def _calculate_correct_fan(tensor, mode):
    ...
    https://github.com/pytorch/pytorch/blob/master/torch/nn/init.py#L337
    """
    mode = mode.lower()
...
def get_sinusoidal_positional_embedding(
    ...
    timesteps = timesteps.to(torch.get_default_dtype())  # type: ignore
    device = timesteps.device

    half_dim = embedding_dim // 2
    emb = math.log(10000) / (half_dim - 1)
    emb = torch.exp(torch.arange(half_dim, dtype=torch.float, device=device) * -emb)  # type: ignore  # noqa
    emb = timesteps[:, None] * emb[None, :]  # type: ignore
    emb = torch.cat(  # type: ignore
        [torch.sin(emb), torch.cos(emb)], dim=1  # type: ignore
    )  # bsz x embd  # type: ignore
    if embedding_dim % 2 == 1:  # zero pad
        emb = F.pad(emb, (0, 1), "constant", 0)  # type: ignore
    assert list(emb.size()) == [timesteps.size(0), embedding_dim]  # type: ignore
    return emb
