...
class Encoder(nn.Module):
  def __init__(self,channels=[3,64,128,256,512,1024], kernel_size=3):
    ...
    self.encode_blocks = nn.ModuleList(
        [Block(channels[i],channels[i+1], kernel_size) for i in range(len(channels)-1)]
        )
...
class Decoder(nn.Module):
  def __init__(self,channels=[1024,512,256,128,64],skip=[]):
    ...
    self.upconvs = nn.ModuleList(
        [nn.ConvTranspose2d(channels[i],channels[i+1],2,2) for i in range(len(channels)-1)] # kernel_size used to be 4
        )
    self.decode_blocks = []
...
class UNet(nn.Module):
  def __init__(
      self,
      encoder_channels=[1,32,64,128,256,512], # 1,32,64,128,256,512 # 1,64,128,256
