...
def fft2c_old(data: torch.Tensor, norm: str = "ortho") -> torch.Tensor:
    ...
    normalized = True if norm == "ortho" else False  # type: ignore

    data = ifftshift(data, dim=[-3, -2])
    data = torch.fft(data, 2, normalized=normalized)  # type: ignore
    data = fftshift(data, dim=[-3, -2])

    return data


def ifft2c_old(data: torch.Tensor, norm: str = "backward") -> torch.Tensor:
    """
    ...
    data = ifftshift(data, dim=[-3, -2])
    data = torch.ifft(data, 2, normalized=normalized)  # type: ignore
    data = fftshift(data, dim=[-3, -2])
