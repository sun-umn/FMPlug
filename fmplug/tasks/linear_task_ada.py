...
def solve(config_name: str) -> None:
    ...
    wandb_instance = wandb.init(  # type: ignore  # noqa
    ...
    for gt_path in gt_paths:
        ...
        def f(x, t, prompt_embedding, pooled_embedding, device):  # type: ignore
            with torch.amp.autocast(device.type, dtype=data_type):  # type: ignore
                result = transformer(
                    hidden_states=x,
                    timestep=t,
        ...
        if optimizer_select == "adam":
            ...
            optimizer_z = torch.optim.LBFGS(
                [z], lr=lr, max_iter=50, history_size=50, line_search_fn="strong_wolfe"
            )
        ...
        def closure_z():
            ...
            decoded_output = torch.sin(decoded_output)
            if measure_config["operator"]["name"] == "inpainting":
                operator_decoded_output = operator.forward(output, mask=mask)
            else:
                operator_decoded_output = operator.forward(decoded_output)
        ...
        plt.tight_layout()
        fig.savefig(
            os.path.join(save_dir, rel_dir, "best_img.png"),
            bbox_inches="tight",
        )
        plt.close()
