...
PROJECT_NAME = "FMPlug-W-R-DIP-Residual"
...
def solve(config_name: str) -> None:
    ...
    residual_type = fmplug_config["residual_type"]
    ...
    for gt_path in gt_paths:
        ...
        for iterator in tqdm.tqdm(range(epochs)):
            ...
            def closure_mix():
                ...
                with torch.amp.autocast("cuda", dtype=data_type):
                    ...
                    if measure_config['operator']['name'] == 'inpainting':
                        operator_decoded_output = operator.forward(decoded_output, mask=mask)
                        operator_residual_output = operator.forward(decoded_output + img_residual, mask=mask)
                    else:
                        operator_decoded_output = operator.forward(decoded_output)
                        operator_residual_output = operator.forward(decoded_output + img_residual)

                    loss = criterion(operator_decoded_output, y_n) + criterion(operator_residual_output, y_n)
                    # loss = criterion(operator_residual_output, y_n)
                    encoded = encode(decoded_output)
                    loss += vae_weight * criterion(x_t, encoded)
                    loss += lpips_weight * percep_loss_fn((operator_decoded_output + 1.0) / 2.0, (y_n + 1.0) / 2.0)
                    loss += TV_reg_weight * L1_tv(decoded_output) / L2_tv(decoded_output) / decoded_output.numel() / 2.0
            ...
            if optimizer_select == "adam":
                loss = optimizer.step(closure_mix)
            ...
            with torch.no_grad():
                ...
                output = (decoded_output + img_residual).detach().float()
                ...
                img_residual_numpy = np.clip((img_residual.detach().float().cpu().squeeze().numpy() + 1) / 2, 0, 1)
                img_residual_numpy = np.transpose(
                    img_residual_numpy, (1, 2, 0)
                )
                # calculate psnr
                ...
                if iterator % 10 == 0:
                    ...
                    save_png_cv2(img_residual_numpy, os.path.join(save_dir, rel_dir, f"img_residual_{str(iterator)}.png"))
        ...
        save_png_cv2(img_residual_numpy, os.path.join(save_dir, rel_dir, f"img_residual_last.png"))
