...
def solve(config_name: str) -> None:
    ...
    for gt_path in gt_paths:
        ...
        if optimizer_select == "adam":
            ...
            # params_group3 = {'params': res_y, 'lr': lr_z[0]/1E1}
            params_group4 = {'params': trainable_kernel, 'lr': 0.05}
            if "turbulence" in task:
                params_group5 = {'params': trainable_tilt, 'lr': 1E-7}
                optimizer = torch.optim.AdamW([params_group1, params_group2, params_group4, params_group5])
            else:
                optimizer = torch.optim.AdamW([params_group1, params_group2, params_group4])
        ...
        for iterator in tqdm.tqdm(range(epochs)):
            ...
            def closure():
                ...
                with torch.amp.autocast("cuda", dtype=data_type):
                    ...
                    temp_z = (1-temp_alpha) * z + temp_alpha * (latent_y + res_y)
                    ...
                    operator_decoded_output = operator.forward(decoded_output, out_k, trainable_tilt)
