...
from fmplug.utils.lowrank_update import LowRankPromptAdapter
...
PROJECT_NAME = "FMPlug-W-R-Prompt-Tuning"
...
def solve(config_name: str) -> None:
    ...
    guidance_scale_init = fmplug_config["guidance_scale_init"]
    guidance_scale_max = fmplug_config["guidance_scale_max"]
    lr_z = fmplug_config["lr_z"]
    lr_dec = fmplug_config["lr_dec"]
    lr_alpha_ada = fmplug_config["lr_alpha_ada"]
    lr_emb = fmplug_config["lr_emb"]
    ...
    for gt_path in gt_paths:
        ...
        def checkpointed_integrate(z, prompt_embeds, pooled_prompt_embeds, negative_prompt_embeds, negative_pooled_prompt_embeds):
        ...
        prompt_embeds = torch.nn.Parameter(prompt_embeds.clone().detach().to(device), requires_grad=True)
        pooled_prompt_embeds = torch.nn.Parameter(pooled_prompt_embeds.clone().detach().to(device), requires_grad=True)
        
        negative_prompt_embeds = torch.nn.Parameter(negative_prompt_embeds.clone().detach().to(device), requires_grad=True)
        negative_pooled_prompt_embeds = torch.nn.Parameter(negative_pooled_prompt_embeds.clone().detach().to(device), requires_grad=True)

        
        # frozen base embedding from text encoder
        # Wrap trainable adapters
        # adapter_tokens = LowRankPromptAdapter(prompt_embeds.shape[-1], r=16).to(device)
        # adapter_pooled = LowRankPromptAdapter(pooled_prompt_embeds.shape[-1], r=16).to(device)

        # Freeze the original embeddings
        ...
        if optimizer_select == "adam":
            ...
            # params_group3 = {'params': chain(adapter_tokens.parameters(), adapter_pooled.parameters()), 'lr': lr_emb[0]}
            params_group3 = {'params': [prompt_embeds, pooled_prompt_embeds], 'lr': lr_emb[0]}
            params_group4 = {'params': [negative_prompt_embeds, negative_pooled_prompt_embeds], 'lr': lr_emb[0]/10.}
            ...
            #     params_group5 = {'params': res_y, 'lr': lr_z[0]/1E3}
                # optimizer = torch.optim.AdamW([params_group1, params_group2, params_group3, params_group4, params_group5])
        ...
        for iterator in tqdm.tqdm(range(epochs)):
            ...
            if finetune_decoder_blocks_interval > 0 and iterator > 0 and iterator % finetune_decoder_blocks_interval == 0 and iterator < finetune_decoder_blocks_interval * len(lr_dec):
                ...
                if block_to_unfreeze_idx < num_decoder_blocks:
                    ...
                    optimizer.param_groups[2]['lr'] = lr_emb[block_to_unfreeze_idx+1]
                    optimizer.param_groups[3]['lr'] = lr_emb[block_to_unfreeze_idx+1]/10.
                    if not is_calibrate:
                        optimizer.param_groups[-1]['lr'] = lr_z[block_to_unfreeze_idx+1]/1E3
            ...
            def closure():
                ...
                with torch.amp.autocast("cuda", dtype=data_type):
                    ...
                    # train_prompt_embeds = adapter_tokens(prompt_embeds)
                    # train_pooled_embeds = adapter_pooled(pooled_prompt_embeds)
                    x_t = checkpoint(checkpointed_integrate, temp_z, prompt_embeds, pooled_prompt_embeds, negative_prompt_embeds, negative_pooled_prompt_embeds)
            ...
            if optimizer_select == "adam":
                ...
                if iterator <= 0.8 * es_min_epochs:
                    guidance_scale = (guidance_scale_max - guidance_scale_init) * (iterator / (0.8 * es_min_epochs)) + guidance_scale_init
