...
def solve(config_name: str) -> None:
    ...
    for gt_path in gt_paths:
        ...
        if optimizer_select == "adam":
            ...
            optimizer_k = torch.optim.LBFGS([trainable_kernel], lr=0.1, max_iter=20, history_size=20, line_search_fn="strong_wolfe")
            optimizer_t = torch.optim.LBFGS([trainable_tilt], lr=1E-7, max_iter=20, history_size=20, line_search_fn="strong_wolfe")
            # params_group1 = {'params': trainable_kernel, 'lr': 0.1}
            # params_group2 = {'params': trainable_tilt, 'lr': 1E-7}
            # optimizer = torch.optim.AdamW([params_group1, params_group2])
            # optimizer_t = torch.optim.AdamW([params_group1, params_group2])
        ...
        for iterator in tqdm.tqdm(range(epochs)):
            ...
            if optimizer_select == "adam":
                ...
                loss = optimizer_k.step(closure_z)
                loss = optimizer_t.step(closure_z)
                # loss = optimizer.step(closure)
