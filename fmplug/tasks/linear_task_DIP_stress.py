...
def solve(config_name: str) -> None:
    ...
    for gt_path in gt_paths:
        for attempt in range(100):
            ...
            os.makedirs(os.path.join(save_dir, str(attempt), rel_dir), exist_ok=True)
            ...
            def dip_closure():
                ...
                with torch.amp.autocast("cuda", dtype=data_type):
                    ...
                    if measure_config['operator']['name'] == 'inpainting':
                        ...
                        operator_dip_output = operator.forward(img_dip_rep)
            ...
            for iterator in tqdm.tqdm(range(epochs)):
                ...
                with torch.no_grad():
                    ...
                    # Append or create
                    log_path = os.path.join(save_dir, str(attempt), rel_dir, "history.csv")
                    ...
                    if early_stop_indicator.get_flag() == False:
                        ...
                        visualize_image(ref_numpy, y_n_numpy, best_img, os.path.join(save_dir, str(attempt), rel_dir, f"img_diff_es_{str(iterator-es_window_size+min_index)}.png"))
                        np.save(os.path.join(save_dir, str(attempt), rel_dir, f"reconstruction_es_{str(iterator-es_window_size+min_index)}.npy"), es_image)
                        save_png_cv2(es_image, os.path.join(save_dir, str(attempt), rel_dir, f"reconstruction_es_{str(iterator-es_window_size+min_index)}.png"))
                        break


            visualize_image(ref_numpy, y_n_numpy, best_img, os.path.join(save_dir, str(attempt), rel_dir, f"img_diff_best.png"))
            visualize_image(ref_numpy, y_n_numpy, output_numpy, os.path.join(save_dir, str(attempt), rel_dir, f"img_diff_last.png"))
            
            # Save the raw data as well
            np.save(os.path.join(save_dir, str(attempt), rel_dir, "gt.npy"), ref_numpy)
            np.save(os.path.join(save_dir, str(attempt), rel_dir, "measurement.npy"), y_n_numpy)
            np.save(os.path.join(save_dir, str(attempt), rel_dir, f"reconstruction_best_{str(best_epoch)}.npy"), best_img)
            np.save(os.path.join(save_dir, str(attempt), rel_dir, f"reconstruction_last.npy"), output_numpy)
            
            save_png_cv2(ref_numpy, os.path.join(save_dir, str(attempt), rel_dir, "gt.png"))
            save_png_cv2(y_n_numpy, os.path.join(save_dir, str(attempt), rel_dir, "measurement.png"))
            save_png_cv2(best_img, os.path.join(save_dir, str(attempt), rel_dir, f"reconstruction_best_{str(best_epoch)}.png"))
            save_png_cv2(output_numpy, os.path.join(save_dir, str(attempt), rel_dir, f"reconstruction_last.png"))
            
            
            with open(os.path.join(save_dir, str(attempt), rel_dir, "prompt.txt"), "w") as file:
                file.write(prompt)

            # Save the file
            config_filename = os.path.join(save_dir, str(attempt), rel_dir, "config.yaml")
