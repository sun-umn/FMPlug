...
niqe_metric = NIQE(
    data_range=255.0
).cuda()  # Initialize NIQE metric, assuming data_range 0-255
...
def preprocess_image_for_lpips_dists(img_np):
    ...
    img_tensor = (
        torch.from_numpy(img_np.astype(np.float32)).permute(2, 0, 1).unsqueeze(0)
    )
    ...
    if os.path.isdir(subdir_path):
        ...
        if os.path.exists(gt_path) and os.path.exists(recon_path):
            ...
            if img_gt.shape != img_recon.shape:
                print(
                    f"Warning: Image dimensions do not match for {subdir}. Skipping. {img_gt.shape} vs {img_recon.shape}"
                )
                continue

            # Ensure uint8 for PSNR and SSIM
            img_gt_uint8 = (
                (np.clip(img_gt, 0.0, 1.0) * 255).astype(np.uint8)
                if img_gt.max() <= 1.0
                else img_gt.astype(np.uint8)
            )
            img_recon_uint8 = (
                (np.clip(img_recon, 0.0, 1.0) * 255).astype(np.uint8)
                if img_recon.max() <= 1.0
                else img_recon.astype(np.uint8)
            )
            ...
            if img_gt_uint8.ndim == 2:  # Grayscale
                ...
                ssim_value = compare_ssim(
                    img_gt_uint8, img_recon_uint8, multichannel=True, data_range=255
                )
            ...
            lpips_value = lpips_loss_fn(
                img_gt_lpips_dists, img_recon_lpips_dists
            ).item()
            lpips_scores.append(lpips_value)

            dists_value = dists_loss_fn(
                img_gt_lpips_dists, img_recon_lpips_dists
            ).item()
            ...
            img_recon_tensor_niqe = (
                torch.from_numpy(preprocess_image_for_fid(img_recon))
                .permute(2, 0, 1)
                .unsqueeze(0)
                .cuda()
            )
...
if gt_images_for_fid and recon_images_for_fid:
    with tempfile.TemporaryDirectory() as gt_temp_dir, tempfile.TemporaryDirectory() as recon_temp_dir:
...
print(
    "MUSIQ, MANIQA, CLIPIQA: These metrics typically require specific pre-trained deep learning models and their associated libraries, which are not readily available for direct integration without further setup or installation of specialized packages. They are currently placeholders."
)
