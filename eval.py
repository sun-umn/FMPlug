...
# Load .npy arrays
# img1_path = '/scratch.global/wan01530/FMPlug/experiments/ae41v7ne/ground_truth_image.npy'  # noqa
# img2_path = '/scratch.global/wan01530/FMPlug/experiments/ae41v7ne/reconstructed_image.npy'  # noqa
...
if img1.dtype != np.uint8:
    img1_uint8 = (
        (np.clip(img1, 0.0, 1.0) * 255).astype(np.uint8)
        if img1.max() <= 1.0
        else img1.astype(np.uint8)
    )
    img2_uint8 = (
        (np.clip(img2, 0.0, 1.0) * 255).astype(np.uint8)
        if img2.max() <= 1.0
